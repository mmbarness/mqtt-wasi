...
impl TraceContext {
    ...
    pub fn new_root(trace_id: [u8; 16], span_id: [u8; 8]) -> Option<Self> {
    ...
    pub fn child(&self, span_id: [u8; 8]) -> Option<Self> {
    ...
    /// Parse W3C Trace Context version 00. Future versions are rejected rather
    /// than interpreted with v00 assumptions.
...
mod tests {
    ...
    fn inject_extract_properties() {
        let ctx = TraceContext::new_root([0xAA; 16], [0xBB; 8]).unwrap();
    ...
    fn child_preserves_trace_id() {
        let parent = TraceContext::new_root([0x11; 16], [0x22; 8]).unwrap();
        let child = parent.child([0x33; 8]).unwrap();
    ...
    fn invalid_traceparent() {
        ...
        assert!(TraceContext::parse_traceparent(
            "ff-4bf92f3577b6a27bc4c989d35b8e7e00-e457b5a2e4d86bd1-01",
            None
        )
        .is_none());
    }
    ...
    #[test]
    fn rejects_zero_trace_and_span_identifiers() {
        assert!(TraceContext::new_root([0; 16], [1; 8]).is_none());
        assert!(TraceContext::new_root([1; 16], [0; 8]).is_none());

        let root = TraceContext::new_root([1; 16], [1; 8]).unwrap();
        assert!(root.child([0; 8]).is_none());
        assert!(TraceContext::parse_traceparent(
            "00-00000000000000000000000000000000-e457b5a2e4d86bd1-01",
            None
        )
        .is_none());
        assert!(TraceContext::parse_traceparent(
            "00-4bf92f3577b6a27bc4c989d35b8e7e00-0000000000000000-01",
            None
        )
        .is_none());
    }
